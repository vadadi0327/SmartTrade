...
from scipy.stats import norm # Required for Probability of Profit math
...
if "tickers" not in st.session_state:
    st.session_state.tickers = ["RKLB", "PLTR", "CRCL", "CRWV", "OKLO", "SMCI", "XE", "GLW", "RGTI"]
...
with st.sidebar:
    try:
        nq_data = yf.download("NQ=F", period="5d", progress=False)
        if isinstance(nq_data.columns, pd.MultiIndex):
            nq_data.columns = nq_data.columns.droplevel(1)
        ...
        with tabs[idx]:
            ...
            try:
                ...
                if df.empty:
                    ...
                    # --- LIVE PRICE INJECTION FIX ---
                    if not live_df.empty and not live_df['Close'].dropna().empty:
                        curr_price = float(live_df['Close'].dropna().iloc[-1])
                        # Force the historical dataframe to adopt the live extended-hours price
                        # This updates the line chart AND calibrates the RSI/MACD to the live tick
                    ...
                    # --- Calculate Historical Volatility (HV) for Options Math ---
                    ...
                    try:
                        ...
                        if news:
                            ...
                            if pos_count > neg_count:
                                news_score = 1.0
                                news_sentiment_reason = "Bullish Catalysts"
                            elif neg_count > pos_count:
                                news_score = -1.0
                                news_sentiment_reason = "Bearish Headwinds"
                    except Exception: pass
                    
                    # 2. Insider/Hedge Fund Sentiment
                    insider_score = 0.0
                    ...
                    try:
                        ...
                        if insider_df is not None and not insider_df.empty:
                            ...
                            if recent_buys > recent_sells:
                                insider_score = 1.0
                                insider_sentiment_reason = "Bullish (Accumulation)"
                            elif recent_sells > recent_buys:
                                insider_score = -1.0
                                insider_sentiment_reason = "Bearish (Distribution)"
                            else:
                    ...
                    with col_strat2:
                        ...
                        if score >= 0.5 or score <= -0.5:
                            if st.button(f"Generate 25% Min ROI & POP% for {ticker}", key=f"btn_{ticker}"):
                                with st.spinner("Calculating Probability of Profit (POP) using Black-Scholes math..."):
                                    exps = tk.options
                                    ...
                                    if not exps:
                                        ...
                                        if not valid_exps:
                                            ...
                                            for exp, dte in valid_exps[:4]:
                                                ...
                                                opts = chain.calls if score > 0 else chain.puts
                                                if not opts.empty:
                                                    opts['expiration'] = exp
                                                    opts['dte'] = max(dte, 1)
                                                    all_opts = pd.concat([all_opts, opts], ignore_index=True)
                    ...
                    with st.expander("🕵️‍♂️ Insider Trading & Corporate News (Dates Included)"):
                        ...
                        with col_ins:
                            ...
                            if insider_df is not None and not insider_df.empty:
                                ...
                                st.info("No recent insider transactions found on Yahoo Finance for this ticker.")
